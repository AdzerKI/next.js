...
Your existing `fetch` and `unstable_cache` caching keeps working. Migrating `unstable_cache` is optional and can happen later.
...
Fully migrate routes that use `dynamic = 'force-static'` or `dynamic = 'error'` in this first step. [`instant = false`](#opting-out-of-validation) only disables validation. It does not preserve the route's previous static behavior, so blocking work can move to request time and reduce how much UI Next.js can prefetch.

1. **Enable the flag and migrate each route segment config.** Follow the relevant sections below for [`dynamic = "force-dynamic"`](#dynamic--force-dynamic), [`dynamic = "force-static"`](#dynamic--force-static), [`dynamic = "error"`](#dynamic--error), [`revalidate`](#revalidate), and [`fetchCache`](#fetchcache). For routes with dynamic params, also follow the [`generateStaticParams`](#generatestaticparams-and-dynamicparams) guidance. Routes that still render instantly need no further work.
...
**Remove the config.** Without Cache Components, `dynamic = 'force-dynamic'` rendered the entire route for every request and made its `fetch` requests uncached. With Cache Components, uncached data and [Request-time APIs](/docs/app/glossary#request-time-apis) already run at request time. Next.js prerenders any static or cached UI around them.
...
**Remove the config and keep the route fully prerendered.** Without Cache Components, `dynamic = 'force-static'` forced the route to prerender, cached its data, and made `cookies()`, `headers()`, and `useSearchParams()` return empty values.

Cache uncached data with [`use cache`](/docs/app/api-reference/directives/use-cache) and a suitable [`cacheLife`](/docs/app/api-reference/functions/cacheLife). After removing the config, Request-time APIs read actual request values. Remove those dependencies if the route must remain fully prerendered.
...
**Remove the config and keep the route fully prerendered.** Without Cache Components, `dynamic = 'error'` forced prerendering, cached `fetch` requests by default, and failed when the route used uncached data or a Request-time API.

If the route has no uncached or request-time work, no replacement is needed. Otherwise, cache the data with [`use cache`](/docs/app/api-reference/directives/use-cache) and [`cacheLife`](/docs/app/api-reference/functions/cacheLife), and remove Request-time API usage.

## `revalidate`

**Replace positive revalidation times with `cacheLife`.** Without Cache Components, `revalidate` set the default revalidation time for an entire route. With Cache Components, place [`use cache`](/docs/app/api-reference/directives/use-cache) around the data or UI that should be cached, then set its lifetime with [`cacheLife`](/docs/app/api-reference/functions/cacheLife).
...
For `revalidate = 0`, remove the config. Uncached data already runs at request time with Cache Components. Keep data outside a `use cache` scope when it must run for every request.

## `fetchCache`

**Remove the config and set caching at the data source.** Without Cache Components, `fetchCache` changed or restricted the `cache` option for every `fetch` request in a route. With Cache Components, fetches are uncached unless they run inside a `use cache` scope.

For `default-cache`, `only-cache`, or `force-cache`, wrap the data that should remain cached in [`use cache`](/docs/app/api-reference/directives/use-cache). For `default-no-store`, `only-no-store`, or `force-no-store`, leave the data outside a cache scope. If `force-no-store` previously overrode an explicit cache option, remove that option as well.
