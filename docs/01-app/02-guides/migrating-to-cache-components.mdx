...
Routes that used `dynamic = 'force-static'` or `dynamic = 'error'` should stay prerendered. Replacing either config with `instant = false` can move rendering to request time and reduce how much UI a link can prefetch, even when the route's data remains cached.
...
**Remove the config, then choose how the request-time work should render.**

Without Cache Components, `dynamic = 'force-dynamic'` renders the entire route for every request. Cache Components does not support this route-level switch. Removing it moves the route to the Partial Prerendering model. Static and cached work can become part of the static shell, while uncached data and [Request-time APIs](/docs/app/glossary#request-time-apis) run at request time.

If request-time work blocks the route from prerendering, choose the behavior you need:

- Cache reusable work with [`use cache`](/docs/app/api-reference/directives/use-cache).
- Wrap request-time work in [`<Suspense>`](https://react.dev/reference/react/Suspense) so the rest of the route can prerender.
- Set [`instant = false`](#opting-out-of-validation) when the segment should stay blocking during an incremental migration.

If the route does not use a Request-time API but still needs a real request before rendering, call [`connection()`](/docs/app/api-reference/functions/connection) before the request-specific work. This preserves the request boundary that `force-dynamic` previously provided.
...
**Remove the config and preserve the route's static contract.** If the route has no uncached or request-time work, it continues to prerender without a replacement.

Cache uncached data with [`use cache`](/docs/app/api-reference/directives/use-cache) and [`cacheLife`](/docs/app/api-reference/functions/cacheLife). Remove Request-time API usage, because `dynamic = 'error'` previously rejected it. Setting `instant = false` instead allows the route to block and changes its rendering behavior.
